...
class Cast
{
	...
	private static var _notClasses:StringMap<Bool> = new StringMap();
	private static var _classes:StringMap<Dynamic> = new StringMap();
	...
	public static function tryColor(data:Dynamic):UInt
	{
		...
		if (Std.is(data,String))
		{
			...
			if (_colorNames == null)
			{
				_colorNames = new haxe.ds.StringMap();
