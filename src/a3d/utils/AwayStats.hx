package a3d.utils
{
	...
	class AwayStats extends Sprite
	{
		 ...
		 * respectively.
		 */
		public function AwayStats(view3d:View3D = null, minimized:Bool = false, transparent:Bool = false, meanDataLength:UInt = 0, enableClickToReset:Bool = true, enableModifyFrameRate:Bool =
			true)
		{
			super();
...
//				_poly_tf.text = _rfaces.toString().concat(' / ', _tfaces); // TODO: Total faces not yet available in 4.x
		...
		{
			stage.frameRate -= Math.floor((_diagram.mouseY - _dia_bmp.height * 0.5) * 0.2);
		}
