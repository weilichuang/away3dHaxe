...
class BitmapFilterEffects extends Sprite
{
	...
	private static var _sharpenFilters:Array<BitmapFilter> = [new ShaderFilter(_sharpenShader)];

	//normal map vars
	private static var _normalMapShader:Shader = new Shader(new NormalMapClass());
	private static var _normalMapFilters:Array<BitmapFilter> = [new ShaderFilter(_normalMapShader)];

	//outline vars
	private static var _outlineShader:Shader = new Shader(new OutlineClass());
	private static var _outlineFilters:Array<BitmapFilter> = [new ShaderFilter(_outlineShader)];
	 ...
	 * create vector sky
	 */
	static public function vectorSky(zenithColor:UInt, horizonColor:UInt, nadirColor:UInt, quality:Int = 8):BitmapCubeTexture
	{
		var xl:Int = 128 * quality;
		var pinch:Int = Std.int(xl / 3.6);

		// sky color from bottom to top;
		var color:Vector<Int> = Vector.ofArray([lighten(nadirColor, 50), darken(nadirColor, 25), darken(nadirColor, 5), horizonColor, horizonColor, zenithColor, darken(zenithColor, 25), darken(zenithColor,
			50)]); // clear
		...
		g.graphics.beginGradientFill(GradientType.LINEAR, [color[1], color[2], color[3], color[4], color[5], color[6]], [1, 1, 1, 1, 1, 1], [90, 110, 120, 126, 180, 230], matrix);
		g.graphics.drawRect(0, 0, xl, xl);
		g.graphics.endFill();
		var displacement_map:DisplacementMapFilter = new DisplacementMapFilter(pinchMap(xl, xl), new Point(0, 0), 4, 2, 0, pinch, DisplacementMapFilterMode.CLAMP);
		...
		g.graphics.beginGradientFill(GradientType.RADIAL, [color[7], color[6]], [1, 1], [0, 255], matrix);
		...
		matrix = new Matrix();
		matrix.createGradientBox(xl, xl, 0, 0, 0);
		g.graphics.beginGradientFill(GradientType.RADIAL, [color[0], color[1]], [1, 1], [0, 255], matrix);
		g.graphics.drawEllipse(0, 0, xl, xl);
		g.graphics.endFill();
		floor.draw(g);
	 ...
	 * lighten color
	 */
	static public function lighten(color:UInt, percent:Float):Int
	{
		...
		return rgbToHex(Std.int(channel + rgb[0] * factor), 
		Std.int(channel + rgb[1] * factor), 
		Std.int(channel + rgb[2] * factor));
	}
	...
	{
		return Vector.ofArray([Std.int((color & 0xff0000) >> 16), Std.int((color & 0x00ff00) >> 8), Std.int(color & 0x0000ff)]);
	}
}

@:file("pb/Sharpen.pbj") class SharpenClass extends ByteArray { }
@:file("pb/NormalMap.pbj") class NormalMapClass extends ByteArray { }
@:file("pb/Outline.pbj") class OutlineClass extends ByteArray { }