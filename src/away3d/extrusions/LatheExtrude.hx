...
// non distributed thickness
    ...
    private function buildThicknessPoints(aPoints:Vector<Vector3D>, thickness:Float, prop1:String, prop2:String):Vector<FourPoints> 
	{
		...
		{
            if (Reflect.field(aPoints[i], prop1) == 0 && 
				Reflect.field(aPoints[i], prop2) == 0) 
			{
				Reflect.setField(aPoints[i], prop1, EPS);
			}
				
            if (Reflect.field(aPoints[i + 1], prop2) != null && 
				Reflect.field(aPoints[i], prop2) == Reflect.field(aPoints[i + 1], prop2)) 
			{
                Reflect.setField(aPoints[i + 1], prop2, Reflect.field(aPoints[i + 1], prop2) + EPS);
            }
            if (Reflect.field(aPoints[i], prop1) != null &&
				Reflect.field(aPoints[i], prop1) == Reflect.field(aPoints[i + 1], prop1))
			{
