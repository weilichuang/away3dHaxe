...
class Ray
{
	...
	*
	* @return		Bool		If the ray intersects the sphere
	*/
	...
	*@param		sPos			Vector3D. 		The position of the sphere.
	*@param		radius		Number.		The radius of the sphere.
	*@param		bNearest	[optional] Bool. If the ray traverses the sphere and if true the returned hit is the nearest to ray origin. Default is true.
	*@param		bNormal		[optional] Bool. If the returned vector is the normal of the hitpoint. Default is false.
	*
	...
	* example of a ray triggered from mouse
		...
		//hittest
		Debug.trace("Ray intersects sphere :"+ _ray.intersectsSphere(pMouse, dir, spherePosition, 500) );
	...
	* example: fire a ray from camera position to 0,0,0 and test if it hits the triangle.
		...
		Debug.trace("intersect ray: "+intersect);
