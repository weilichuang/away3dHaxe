...
class ParticleFollowState extends ParticleStateBase
{
	...
	override public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, animationSubGeometry:AnimationSubGeometry, animationRegisterCache:AnimationRegisterCache, camera:Camera3D):Void
	{
		...
		else if (_particleFollowNode.usesRotation)
		{
			...
			animationSubGeometry.activateVertexBuffer(animationRegisterCache.getRegisterIndex(_animationNode, ParticleFollowNode.FOLLOW_ROTATION_INDEX), _particleFollowNode.dataOffset, stage3DProxy,
				Context3DVertexBufferFormat.FLOAT_3);
		}
	...
	private function processPositionAndRotation(currentTime:Float, deltaTime:Float, animationSubGeometry:AnimationSubGeometry):Void
	{
		...
		for (i in 0...len)
		{
			...
			if (t - deltaTime <= 0)
			{
				...
				if (vertexData[inc] != interpolatedPos.x || vertexData[inc + 1] != interpolatedPos.y || vertexData[inc + 2] != interpolatedPos.z || vertexData[inc + 3] != interpolatedRotation.x ||
